...
""")
    ...
    def test_default_value_retrieval(self, config_parser):
        ...
        with pytest.raises(Exception, match="Default value 'nonexistent_setting' not found"):
...
""")
            ...
            with patch.object(config_parser.config, 'get') as mock_get:
                ...
                with patch.object(config_parser.config, 'has_section', return_value=True):
                    with patch.object(config_parser.config, 'has_option', return_value=True):
                        # Should raise exception for invalid timeout format
                        with pytest.raises(Exception, match="Invalid timeout configuration"):
